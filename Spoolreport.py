...
# -------- HELPERS (As per your original program) --------
def clean_val(x):
...
if df is not None:
    ...
    if usr_no:
        ...
        if usr_df.empty:
            ...
            # કોલમ સુરક્ષિત રીતે વેરિફાય કરવી
            existing_columns = [
                col
            ...
            # 📋 લાઈવ ડેટા પ્રીવ્યૂ (સેફ રીત - એરર ફ્રી)
            st.subheader("📋 લાઈવ ડેટા પ્રીવ્યૂ")
            preview_df = usr_df.copy()
            # પ્રીવ્યૂ સ્ક્રીન પર ન દેખાતી કોલમ માટે ખાલી કોલમ બનાવી દેવી જેથી KeyError ન આવે
            for col in existing_columns:
                if col not in preview_df.columns and col == "NDT Status":
                    preview_df["NDT Status"] = ""

            # પ્રોપર લુક સાથે પ્રીવ્યૂ ટેબલ
            st.dataframe(
                preview_df[
            ...
            # -------- PDF BUILDER FUNCTION (Your Original Logic) --------
            def generate_pdf_bytes():
                ...
                elements.append(
                    Paragraph(
                        f"NDT CLEARANCE DATE:- {ndt_clearance_date}", extra_style
                    )
                )
