        ...
        with st.form(key="login_form"):
            ...
            if submit:
                if "users" in st.secrets:
                    ...
                    if user in valid_users and str(pwd) == str(valid_users[user]):
...
def get_ndt_info(full_df, lot_no, lot_col, rep_col, date_col, test_type, xr_col=None):
    if not lot_no:
        return None, ""
    ...
    for _, r in matches.iterrows():
        ...
        if rep_no and rep_date:
            ...
            extra_parts = []
            if iso_no: extra_parts.append(iso_no)
            if spool_no: extra_parts.append(spool_no)
            if joint_no: extra_parts.append(joint_no)
...
if df is not None:
    ...
    if usr_no:
        ...
        if usr_df.empty:
            ...
            if view_report:
                required_columns = [
                    "ISO No/Drawing No/Line No", "Joint No.", "Type of Joint",
                    "WELD NPD", "Spool Unique No.", "Induction bend  DPT Lot no",
                    "FIT UP Date", "Welder No", "WELD VISUAL REPORT NO",
                    "VISUAL Date", "DPT LOT NO", "RT REPORT NO", "XR NO",
                    "RT LOT NO", "NDT Status"
                ]
                ...
                def generate_pdf_bytes():
                    ...
                    elements = []
                    title_style = ParagraphStyle('Title', fontSize=14, textColor=HexColor("#2C3E50"), alignment=1, spaceAfter=12)
                    body_style = ParagraphStyle('Body', fontSize=6.5, leading=9, alignment=1)
                    header_style = ParagraphStyle('Header', fontSize=7, leading=8, alignment=1, textColor=colors.whitesmoke, fontName='Helvetica-Bold')
                    ...
                    for _, row in usr_df.iterrows():
                        ...
                        if joint_type == "EB":
                            ...
                            rmk, status = get_ndt_info(full_df, lot_val, "Induction bend  DPT Lot no", "Induction bend  DPT Report No", "Induction bend  DPT Date", "Induction DPT")
                            if lot_val:
                                current_ndt_status = status
                                if rmk: final_remarks.add(rmk)
                            ...
                            try:
                                if pd.isna(raw_percent) or raw_percent == "":
                                    ...
                                    if percent_value > 1: percent_value = percent_value / 100
                            except:
                                percent_value = 0
                            ...
                            if percent_value == 1:
                                ...
                                rmk, status = get_ndt_info(full_df, lot_val, "DPT LOT NO", "DPT REPORT NO", "DPT DATE", "DPT")
                                if lot_val:
                                    current_ndt_status = status
                                    if rmk: final_remarks.add(rmk)
                            ...
                            rmk, status = get_ndt_info(full_df, lot_val, "RT LOT NO", "RT REPORT NO", "RT DATE", "RT", xr_col="XR NO")
                            if lot_val:
                                current_ndt_status = status
                                if rmk: final_remarks.add(rmk)
                        ...
                        for col_name in existing_columns:
                            if col_name == "NDT Status": val = current_ndt_status
                            elif col_name == "DPT LOT NO" and lot_override: val = lot_override
                            else: val = clean_val(row.get(col_name, ""))
                    ...
                    column_widths = []
                    for col in existing_columns:
                        if "ISO No" in col: column_widths.append(130)
                        elif "Date" in col or "DATE" in col: column_widths.append(60)
                        elif "NDT Status" in col: column_widths.append(65)
                        else: column_widths.append(42)

                    table = Table(table_data, colWidths=column_widths, rowHeights=[40]*len(table_data), repeatRows=1)
                    ...
                    if final_remarks:
                        ...
                        for rmk in sorted(final_remarks):
                            elements.append(Paragraph(rmk, ParagraphStyle('RemarkStyle', fontSize=9, fontName='Helvetica-Bold', leading=12)))

                    first_row = usr_df.iloc[0]
                    ndt_clearance_date = clean_val(first_row.get("NDT CLEARANCE", ""))
                    fd_date = clean_val(first_row.get("FD Date", ""))
                    dc_no = clean_val(first_row.get("Shift to Laydown / Painting Yard DC No", ""))

                    elements.append(Spacer(1, 15))
                    extra_style = ParagraphStyle('ExtraStyle', fontSize=9, fontName='Helvetica-Bold', leading=12)
                    elements.append(Paragraph(f"NDT CLEARANCE DATE:- {ndt_clearance_date}", extra_style))
